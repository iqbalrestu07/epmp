...
func Validate(cfg *GeneratorConfig) []error {
	...
	if cfg.Backend.OutputRoot == "" {
		errs = append(errs, fmt.Errorf("backend.output_root is required"))
	}
	if cfg.Backend.ModulePath == "" {
		errs = append(errs, fmt.Errorf("backend.module_path is required"))
	}
	...
	if cfg.Domain.Table == "" {
		errs = append(errs, fmt.Errorf("domain.table is required"))
	}
