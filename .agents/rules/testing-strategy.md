...
> **Override untuk proyek `kes-ref-screening-directus`:** Integration tests dijalankan terhadap Directus instance via Docker Compose + PostgreSQL. Tidak menggunakan Testcontainers atau Firebase Emulator. Lihat `directus-extension-patterns.md` untuk testing approach yang relevan — primary method adalah manual testing via `curl` atau API client terhadap endpoint Directus yang berjalan.
...
- **Naming Convention Example:**
  - **JS/TS:** `*.spec.js` (Unit), `*.integration.spec.js` (Integration)
  ...
  > **Language-specific overrides:** Some ecosystems have different test location conventions. See `architectural-pattern.md` § Test co-location for the authoritative rule.
