...
**Naming Conventions:**
- **Language: English only** — All table names, column names, indexes, and constraints MUST use English words. Never use local language names (e.g., use `unit` not `satuan`, `recommendation` not `rekomendasi`). This ensures international readability and IDE tooling compatibility.
