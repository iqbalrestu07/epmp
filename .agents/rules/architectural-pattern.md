...
1. Search the codebase for existing abstraction patterns by looking for symbols named: `Repository`, `RepositoryImpl`, `Service`, `Handler`, `Sender` (use your available codebase search tool — codebase-memory MCP `search_graph`, grep, IDE symbol search, etc.)
2. Match the style (repository interface in `repository/`, impl in `repository/*_impl.go` for Go; `features/<x>/api` as the I/O module in TypeScript)
...
**Example (TypeScript/React — `features/<x>/api` is the I/O boundary):**

```typescript
// I/O adapter (features/property/api/index.ts) — only place that talks to @/services/api
...
// Consumer (hooks) depends on the api module; tests replace it with a fake
export function useProperty(id?: string) {
  return useQuery({
    queryKey: ["propertys", id],
...
1. **Search existing patterns** (MANDATORY):
   Search the codebase for symbols named: `Repository`, `Service`, `Handler`, `NewModule`
   (use your available codebase search tool — codebase-memory MCP `search_graph`, grep, IDE symbol search, etc.)
...
- "Following pattern from [property, tenant, room] modules"
- "X/Y modules use interface-based repositories"
- "All tests use [fake repository, testutil.Setup, testutil.AssertCRUD] pattern"
...
- E2E tests: isolated from features — in this project the Playwright runner is `frontend/run_e2e_tests.cjs` (see `E2E_TESTING.md`). They cross feature boundaries and belong to none. See Testing Strategy @testing-strategy.md.
- **Project conventions:** Go tests are `*_test.go` next to the code; API integration tests are `<module>/<name>_api_test.go` using `internal/testutil`. See @epmp-module-patterns.md for authoritative guidance.
...
- Go: `go-idioms-and-patterns.md`
- TypeScript: `typescript-idioms-and-patterns.md`

**Universal rule:** In every language, the testability pattern is the same — I/O is behind an interface, business logic is pure, and tests inject a mock/fake implementation. The idiom files describe _how_ to express this pattern in each ecosystem's syntax.
