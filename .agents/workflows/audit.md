...
_Applies to: full-stack projects with frontend + backend_
...
_Applies to: projects using a relational or document database_

- [ ] Verify all tables have required base columns (`id`, `created_at`, `updated_at`)
- [ ] Check all foreign keys have corresponding indexes
- [ ] If using Postgres RLS: verify RLS policies exist on every table storing user data
...
- [ ] No unused top-level dependencies in `go.mod` / `package.json`
...
- [ ] Dependency audit — flag high-severity CVEs:
  - `cd backend && go list -m -u all` and `go run golang.org/x/vuln/cmd/govulncheck@latest ./...`
  - `cd frontend && npm audit --omit=dev`
  - Docker: `docker compose build`, then note base images in `backend/Dockerfile` / `frontend/Dockerfile`
- [ ] For remediation of dependency findings, use the **patch-cve** skill
...
_Applies to: projects with a mobile app and a backend_
...
**Output location:** `epmp-docs/audits/{date}-{scope}.md`
...
1. Create the `epmp-docs/audits/` directory if it doesn't exist
2. Write the findings report to `epmp-docs/audits/{date}-{scope}.md`
...
| B. Database & Schema           | ✅ Checked / ⏭ Skipped (reason) | e.g., reviewed all tables in `backend/migrations/` + repository adapters  |
| C. Configuration & Environment | ✅ Checked                       | e.g., scanned for raw secrets, verified .env.template                     |
| D. Dependency Health           | ✅ Checked                       | e.g., ran govulncheck + npm audit, checked go.mod for unused deps         |
...
> "Fix the critical issues in `epmp-docs/audits/2026-02-16-gatekeeper.md`"
...
- [ ] Findings document saved to `epmp-docs/audits/` in the repo
