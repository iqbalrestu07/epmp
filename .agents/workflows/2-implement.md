...
- Research log exists at `epmp-docs/research-logs/{feature_name}.md`
...
- Go: `*_test.go`
- Frontend: no unit test runner exists — validate via `npm run lint` / `npm run build` and E2E
...
```bash
# Go
cd backend && go test -v ./internal/modules/{module}/...
```
...
```bash
# Run specific test
cd backend && go test -v -run TestCreateTask ./internal/modules/task/...

# Run with coverage
cd backend && go test -cover ./internal/modules/...

# Frontend (no unit test runner exists — use lint/build)
