...
// OccupancyHandler handles HTTP requests for Occupancy resources.
...
func (h *OccupancyHandler) Create(c echo.Context) error {
	orgID := mw.GetOrgID(c)
	if orgID == "" {
...
func (h *OccupancyHandler) GetByID(c echo.Context) error {
	id := c.Param("id")
	orgID := mw.GetOrgID(c)

	result, err := h.svc.GetByID(c.Request().Context(), id, orgID)
	if err != nil {
		return response.NotFound(c, "Occupancy not found")
	}

	return response.OK(c, result)
}

func (h *OccupancyHandler) List(c echo.Context) error {
...
func (h *OccupancyHandler) Update(c echo.Context) error {
	id := c.Param("id")
	orgID := mw.GetOrgID(c)

	var req dto.UpdateOccupancyRequest
	if err := c.Bind(&req); err != nil {
		return response.BadRequest(c, "invalid request body")
	}
...
func (h *OccupancyHandler) Delete(c echo.Context) error {
	id := c.Param("id")
	orgID := mw.GetOrgID(c)
